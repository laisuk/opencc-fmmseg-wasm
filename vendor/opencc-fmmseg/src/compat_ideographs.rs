...
//!
//! The built-in table is loaded from canonical
//! `data/CJK_Compatibility_Ideographs.txt` by default, parsed once, and cached
//! in dense runtime lookup tables with [`std::sync::OnceLock`]. Unmapped
//! compatibility ideographs remain unchanged, as do characters outside the
//! compatibility ideograph ranges.
//!
//! # Cargo feature
//!
//! Enabling the optional `compat-bin` feature switches the built-in runtime
//! loader from the canonical text file to
//! `data/CJK_Compatibility_Ideographs.bin`, a compact generated artifact. The
//! binary file must be regenerated from `CJK_Compatibility_Ideographs.txt` with
//! `dict-generate --compat` whenever the canonical text data changes.
//!
//! The feature model is intentionally binary:
//!
//! - without `compat-bin`, the runtime loads embedded TXT data;
//! - with `compat-bin`, the runtime loads embedded BIN data.
//!
